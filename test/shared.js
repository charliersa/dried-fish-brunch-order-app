...
// ===== 測試層（/test/ 子目錄）=====
// 為了不打擾營業，改版一律先放到 test/ 子目錄驗證，確認沒問題再同步到正式站。
// 只要網址路徑含 /test/ 就自動切成測試模式，同一份程式碼不必改：
//  ① Firestore 集合全部加 test_ 前綴 → 測試訂單不會出現在廚房／收銀／叫號螢幕
//  ② localStorage 的 key 也加前綴 → 同網域不會蓋掉正式站的暫存（進行中訂單、後台登入等）
//  ③ 推播端點清空 → 測試單不會震到店裡與顧客的手機
//  ④ 畫面最上方顯示橘色測試站橫幅，避免誤當正式站使用
const IS_TEST_SITE = (function () {
  try { return location.pathname.indexOf('/test/') !== -1; } catch (e) { return false; }
})();
...
if (IS_TEST_SITE) {
  try {
    ...
    const _get = _proto.getItem, _set = _proto.setItem, _del = _proto.removeItem;
    _proto.getItem = function (k) { return _get.call(this, TEST_PREFIX + k); };
    _proto.setItem = function (k, v) { return _set.call(this, TEST_PREFIX + k, v); };
    _proto.removeItem = function (k) { return _del.call(this, TEST_PREFIX + k); };
  } catch (e) {}
  try {
    // firebase-config.js 比 shared.js 先載入，這裡直接把端點清掉
    if (typeof PUSH_CONFIG !== 'undefined' && PUSH_CONFIG) {
      PUSH_CONFIG.notifyEndpoint = '';
      PUSH_CONFIG.notifyBackendEndpoint = '';
    }
  } catch (e) {}
  const _showTestBanner = function () {
    if (!document.body || document.getElementById('test-site-banner')) return;
    const bar = document.createElement('div');
    bar.id = 'test-site-banner';
    bar.textContent = '🧪 測試站 · 資料與正式站完全分開，不會影響營業';
    bar.style.cssText = 'position:sticky;top:0;z-index:9999;background:#e89a2b;color:#fff;font-weight:900;font-size:13px;text-align:center;padding:7px 10px;';
    document.body.insertBefore(bar, document.body.firstChild);
  };
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', _showTestBanner);
  else _showTestBanner();
}
...
function ensureDb() {
  ...
  try {
    ...
    // 測試站：包一層把集合名稱換成 test_ 開頭，其餘方法原樣轉發
    if (IS_TEST_SITE) {
      return {
        collection: name => db.collection(TEST_PREFIX + name),
